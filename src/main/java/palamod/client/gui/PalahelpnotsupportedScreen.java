...
public class PalahelpnotsupportedScreen extends AbstractContainerScreen<PalahelpnotsupportedMenu> {
	...
	protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
		this.font.draw(poseStack, Component.translatable("gui.palamod.palahelpnotsupported.label_an_error_has_been_encountred"), 14, 39, -26368);
		this.font.draw(poseStack, Component.translatable("gui.palamod.palahelpnotsupported.label_palahelp_error"), 49, 7, -65485);
		this.font.draw(poseStack, Component.translatable("gui.palamod.palahelpnotsupported.label_your_language_is_not_supported"), 12, 53, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.palahelpnotsupported.label_your_can_help_for_translation"), 5, 74, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.palahelpnotsupported.label_on_crowdin"), 4, 86, -12829636);
	}
	...
	public void init() {
		super.init();
		button_crowdin_link = Button.builder(Component.translatable("gui.palamod.palahelpnotsupported.button_crowdin_link"), e -> {
		}).bounds(this.leftPos + 50, this.topPos + 137, 87, 20).build();
		guistate.put("button:button_crowdin_link", button_crowdin_link);
		this.addRenderableWidget(button_crowdin_link);
		button_language = Button.builder(Component.translatable("gui.palamod.palahelpnotsupported.button_language"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 4, this.topPos + 103, 67, 20).build();
		guistate.put("button:button_language", button_language);
		this.addRenderableWidget(button_language);
		button_exit = Button.builder(Component.translatable("gui.palamod.palahelpnotsupported.button_exit"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 76, this.topPos + 103, 46, 20).build();
		guistate.put("button:button_exit", button_exit);
		this.addRenderableWidget(button_exit);
		button_back = Button.builder(Component.translatable("gui.palamod.palahelpnotsupported.button_back"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 135, this.topPos + 103, 46, 20).build();
