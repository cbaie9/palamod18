...
public class ForgepalahelpguiScreen extends AbstractContainerScreen<ForgepalahelpguiMenu> {
	...
	protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
		this.font.draw(poseStack, Component.translatable("gui.palamod.forgepalahelpgui.label_paladium_forge"), 5, 2, -65485);
		this.font.draw(poseStack,

				Palahelpforge0Procedure.execute(entity), 2, 96, -12829636);
		this.font.draw(poseStack,

				Palahelpforge1Procedure.execute(entity), 3, 106, -12829636);
		this.font.draw(poseStack,

				Palahelpforge2Procedure.execute(entity), 3, 117, -12829636);
		this.font.draw(poseStack,

				Palahelpforge3Procedure.execute(entity), 4, 128, -12829636);
	}
	...
	public void init() {
		super.init();
		button_back = Button.builder(Component.translatable("gui.palamod.forgepalahelpgui.button_back"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 2, this.topPos + 14, 45, 20).build();
		guistate.put("button:button_back", button_back);
		this.addRenderableWidget(button_back);
		button_menu = Button.builder(Component.translatable("gui.palamod.forgepalahelpgui.button_menu"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 2, this.topPos + 35, 45, 20).build();
