...
public class Stickgui1Screen extends AbstractContainerScreen<Stickgui1Menu> {
	...
	protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
		this.font.draw(poseStack, Component.translatable("gui.palamod.stickgui_1.label_sticks"), 173, 4, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.stickgui_1.label_les_stick_bton_de_craftent_d"), 8, 15, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.stickgui_1.label_types_diffrent"), 7, 28, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.stickgui_1.label_speed_stick_vous_permet_davoi"), 5, 83, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.stickgui_1.label_craft_de_la_speed_orb"), 257, 124, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.stickgui_1.label_empty"), 343, 137, -12829636);
	}
	...
	public void init() {
		super.init();
		button_item_menu = Button.builder(Component.translatable("gui.palamod.stickgui_1.button_item_menu"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 4, this.topPos + 57, 70, 20).build();
		guistate.put("button:button_item_menu", button_item_menu);
		this.addRenderableWidget(button_item_menu);
		button_menu = Button.builder(Component.translatable("gui.palamod.stickgui_1.button_menu"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 77, this.topPos + 57, 45, 20).build();
		guistate.put("button:button_menu", button_menu);
		this.addRenderableWidget(button_menu);
		button_next_stick = Button.builder(Component.translatable("gui.palamod.stickgui_1.button_next_stick"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 127, this.topPos + 57, 75, 20).build();
