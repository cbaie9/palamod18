...
public class SetspawnCommand {
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("setspawn").requires(s -> s.hasPermission(4)).then(Commands.literal("new").then(Commands.argument("x_pos", DoubleArgumentType.doubleArg())
				.then(Commands.argument("y_pos", DoubleArgumentType.doubleArg(-64, 320)).then(Commands.argument("z_pos", DoubleArgumentType.doubleArg()).then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(arguments -> {
					...
					SetspawndynablockProcedure.execute(world, x, y, z, arguments, entity);
					return 0;
				}))))).then(Commands.literal("oldsys").then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(arguments -> {
					ServerLevel world = arguments.getSource().getLevel();
					double x = arguments.getSource().getPosition().x();
					double y = arguments.getSource().getPosition().y();
					double z = arguments.getSource().getPosition().z();
