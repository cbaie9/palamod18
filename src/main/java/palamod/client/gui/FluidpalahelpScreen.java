...
public class FluidpalahelpScreen extends AbstractContainerScreen<FluidpalahelpMenu> {
	...
	protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
		this.font.draw(poseStack, Component.translatable("gui.palamod.fluidpalahelp.label_fluid"), 85, 5, -13369345);
		this.font.draw(poseStack, Component.translatable("gui.palamod.fluidpalahelp.label_fake_water_un_type_deau_invisi"), 2, 63, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.fluidpalahelp.label_vous_fait_des_dgats_peut_peut"), 2, 76, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.fluidpalahelp.label_mpecher_la_legendary_stone_de_p"), 4, 86, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.fluidpalahelp.label_angelique_water_type_deau_qui"), 4, 122, -12829636);
		this.font.draw(poseStack, Component.translatable("gui.palamod.fluidpalahelp.label_soigne"), 3, 132, -12829636);
	}
	...
	public void init() {
		super.init();
		button_back = Button.builder(Component.translatable("gui.palamod.fluidpalahelp.button_back"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 8, this.topPos + 165, 45, 20).build();
		guistate.put("button:button_back", button_back);
		this.addRenderableWidget(button_back);
		button_menu = Button.builder(Component.translatable("gui.palamod.fluidpalahelp.button_menu"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 57, this.topPos + 165, 45, 20).build();
