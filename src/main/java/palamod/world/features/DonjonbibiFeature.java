...
public class DonjonbibiFeature extends Feature<NoneFeatureConfiguration> {
	public static DonjonbibiFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new DonjonbibiFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("palamod:donjonbibi", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("palamod:donjonbibi", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("palamod", "bibi2"));
		if (template == null)
			return false;
