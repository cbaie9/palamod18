...
public class AuthsafeguiScreen extends AbstractContainerScreen<AuthsafeguiMenu> {
	...
	protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
		this.font.draw(poseStack, new TranslatableComponent("gui.palamod.authsafegui.label_enter_code"), 3, 6, -12829636);
	}
	...
	public void init() {
		...
		code_check = new EditBox(this.font, this.leftPos + 3, this.topPos + 15, 120, 20, new TranslatableComponent("gui.palamod.authsafegui.code_check"));
		code_check.setMaxLength(32767);
		guistate.put("text:code_check", code_check);
		this.addWidget(this.code_check);
		button_open = new Button(this.leftPos + 3, this.topPos + 36, 46, 20, new TranslatableComponent("gui.palamod.authsafegui.button_open"), e -> {
			if (true) {
