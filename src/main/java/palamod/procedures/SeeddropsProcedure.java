...
public class SeeddropsProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		if (((world.getBlockState(new BlockPos(x, y, z))).getBlock() == Blocks.GRASS || (world.getBlockState(new BlockPos(x, y, z))).getBlock() == Blocks.DEAD_BUSH || (world.getBlockState(new BlockPos(x, y, z))).getBlock() == Blocks.TALL_GRASS
				|| (world.getBlockState(new BlockPos(x, y, z))).getBlock() == Blocks.LARGE_FERN) && Math.random() < 0.1) {
			if (world instanceof Level _level && !_level.isClientSide()) {
				ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(PalamodModItems.EGGPLANTSEED.get()));
